...
console.warn(
  'Remove this line, when WME-Bootstrap will fix its syntax. now it causes script error on load, details https://stackoverflow.com/questions/42036349/uncaught-typeerror-intermediate-value-is-not-a-function'
);
...
(function () {
  ...
  function streetToRiver_bootstrap() {
    $(document).on('bootstrap.wme', function () {
      streetToRiver_init();
    });
  }
  ...
  function streetToRiver_init() {
    ...
    function insertButtons() {
      if (W.selectionManager.getSelectedWMEFeatures().length === 0) return;
    ...
    function convertToLandmark(sel, lmtype, isUnlimitedSize) {
      ...
      if (!bAddNew) {
        ...
        while (firstStreetVerticeOutside < streetVertices.length) {
          if (!riverLandmark.geometry.containsPoint(streetVertices[firstStreetVerticeOutside])) break;
      ...
      var first;
      if (bAddNew) first = 0;
      else first = firstStreetVerticeOutside - 1;
      ...
      if (bAddNew) {
        ...
        if (lmtype !== 'OTHER') {
          ...
          W.model.actionManager.add(
            new wazeActionUpdateFeatureAddress(riverLandmark, newAddressAtts, {
        ...
        if (donorLandmark) {
          ...
          for (let i1 = 0; i1 < components.length; i1++) {
            ...
            if (d1 < dist) {
              dist = d1;
              p1[0] = i1;
              if (componentsRL > 0) p1[1] = i1 === 0 ? components.length - 1 : i1 - 1;
              else p1[1] = i1 == components.length - 1 ? 0 : i1 + 1;
            }
          }
          ...
          for (let i1 = 0; i1 < componentsDonor.length; i1++) {
            let d1 = Math.sqrt(
              Math.pow(Math.abs(componentsDonor[i1].x - streetVertices[streetVertices.length - 1].x), 2) + Math.pow(Math.abs(componentsDonor[i1].y - streetVertices[streetVertices.length - 1].y), 2)
            );
            if (d1 < dist) {
              dist = d1;
              p2[0] = i1;
              if (componentsDonorRL > 0) p2[1] = i1 === 0 ? componentsDonor.length - 1 : i1 - 1;
              else p2[1] = i1 == componentsDonor.length - 1 ? 0 : i1 + 1;
            }
          }
          ...
          // добавляем источник
          for (let i1 = 0; i1 <= p1[0]; ++i1) componentsNew.push(components[i1]);

          // добавляем донора
          if (componentsRL < 0) {
            if (componentsDonorRL < 0) {
              // добавляем донора по кругу
              for (let i1 = p2[0]; i1 < componentsDonor.length; ++i1) componentsNew.push(componentsDonor[i1]);

              // ...остаток донора
              for (let i1 = 0; i1 < p2[0]; ++i1) componentsNew.push(componentsDonor[i1]);
            } else {
              // добавляем донора по кругу
              for (let i1 = p2[0]; i1 >= 0; --i1) componentsNew.push(componentsDonor[i1]);

              // ...остаток донора
              for (let i1 = componentsDonor.length - 1; i1 > p2[0]; --i1) componentsNew.push(componentsDonor[i1]);
            }
          } else {
            if (componentsDonorRL < 0) {
              // добавляем донора по кругу
          ...
          // добавляем источник
          for (let i1 = p1[0] + 1; i1 < components.length; ++i1) componentsNew.push(components[i1]);
          ...
          W.model.actionManager.add(
            new wazeActionUpdateFeatureGeometry(riverLandmark, W.model.venues, W.userscripts.toGeoJSONGeometry(undoGeometry), W.userscripts.toGeoJSONGeometry(riverLandmark.geometry))
          );
          W.model.actionManager.add(
            new wazeActionDeleteObject(donorLandmark, W.model.venues, W.userscripts.toGeoJSONGeometry(undoGeometryDonor), W.userscripts.toGeoJSONGeometry(donorLandmark.geometry))
          );
        ...
        if (firstStreetVerticeOutside === 0) firstStreetVerticeOutside = 1;
        ...
        if (bIsOneVerticeStreet) index += 1;
    ...
    function getStreet(segment) {
      if (!segment.attributes.primaryStreetID) return null;
      var street = segment.model.streets.getObjectById(segment.attributes.primaryStreetID);
      return street;
    }

    function getDisplacement(street) {
      if (!street) return getLastRiverWidth(defaultWidth);
      if (!street.attributes.name) return getLastRiverWidth(defaultWidth);
      if (street.attributes.name.match(/^(\d+)(m|м)\b/))
        // TODO make localizable
        return parseInt(RegExp.$1);
      if (street.attributes.name.match(/^(\d+)ft\b/))
        // TODO make localizable
    ...
    function getLastRiverWidth(defaultRiverWidth) {
      if (typeof Storage !== 'undefined') {
        // 2013-06-09: Yes! localStorage and sessionStorage support!
        if (sessionStorage.riverWidth) return Number(sessionStorage.riverWidth);
        else return Number(defaultRiverWidth); // Default river width
      } else {
        // Sorry! No web storage support..
    ...
    function getLastIsUnlimitedSize(defaultValue) {
      if (typeof Storage !== 'undefined') {
        ...
        if (sessionStorage.isUnlimitedSize) return Number(sessionStorage.isUnlimitedSize);
        else return Number(defaultValue); // Default preference
      } else {
        // Sorry! No web storage support..
    ...
    function getLastIsDeleteSegment(defaultValue) {
      if (typeof Storage !== 'undefined') {
        // 2013-10-20: Yes! localStorage and sessionStorage support!
        if (sessionStorage.isDeleteSegment) return Number(sessionStorage.isDeleteSegment);
        else return Number(defaultValue); // Default preference
      } else {
        // Sorry! No web storage support..
        return Number(defaultValue); // Default preference
    ...
    function getLanguage() {
      ...
      if (wmeLanguage === 'editor') wmeLanguage = 'us';
      ...
      switch (getLanguage()) {
        ...
        case 'es-419':
          langText = new Array(
            '',
            'Ancho (metros)',
            'Cree una nueva calle, selecciónela y oprima este botón.',
            'Calle a Río',
            'Tamaño ilimitado',
            '¡No se encontró una calle sin guardar!',
            'Todos los segmentos de la calle adentro del río. No se puede continuar.',
        ...
        case 'fr': // 2014-06-05: French
          langText = new Array(
            '',
            'Largura (mètres)',
            'Crie uma nova rua, a selecione e clique neste botão.',
            'Rue á rivière',
            'Taille illimitée (dangereux)',
            'Pas de nouvelle rue non enregistré trouvée!',
            'Tous les segments de la rue dans la rivière. Vous ne pouvez pas continuer.',
        ...
        case 'ru': // 2014-06-05: Russian
          langText = new Array(
            '',
            'Ширина (в метрах)',
            'Создайте новую дорогу (не сохраняйте), выберите ее и нажмите эту кнопку.',
            'Река',
            'Вся длина',
            'Не выделено ни одной не сохраненной дороги!',
            'Все сегменты дороги находятся внутри реки. Преобразование невозможно.',
        ...
        case 'uk': // 2018-05-03: Ukrainian
          langText = new Array(
            '',
            'Ширина (в метрах)',
            'Створіть нову дорогу (не зберігайте і не знімайте виділення) та натисніть цю кнопку.',
            'Ріка',
            'Безлімітна довжина (небезпечно)',
            'Не виділено жодної збереженої дороги!',
            'Усі сегменти дороги знаходяться всередині ріки. Перетворення неможливе.',
            'Занадто багато сегментів дороги знаходяться всередині ріки. Перетворення неможливе.',
            'Контур',
            'Ліс',
            'Видалити сегмент',
            'Канал'
        ...
        case 'hu': // 2014-07-02: Hungarian
          langText = new Array(
            '',
            'Szélesség (méter)',
            'Hozzon létre egy új utcát, válassza ki, majd kattintson erre a gombra.',
            'Utcából folyó',
            'Korlátlan méretű (nem biztonságos)',
            'Nem található nem mentett és kiválasztott új utca!',
            'Az útszakasz a folyón belül található! Nem lehet folytatni.',
        ...
        case 'cs': // 2014-07-03: Czech
          langText = new Array(
            '',
            'Šířka (metrů)',
            'Vytvořte osu řeky, vyberte segment a stiskněte toto tlačítko.',
            'Silnice na řeku',
            'Neomezená šířka (nebezpečné)',
            'Nebyly vybrány žádné neuložené segmenty!',
            'Všechny segmenty jsou uvnitř řeky! Nelze pokračovat.',
        ...
        case 'pl': // 2014-11-08: Polish - By Zniwek
          langText = new Array(
            '',
            'Szerokość (w metrach)',
            'Stwórz ulicę, wybierz ją i kliknij ten przycisk.',
            'Ulica w Rzekę',
            'Nieskończony rozmiar (niebezpieczne)',
            'Nie znaleziono nowej i niezapisanej ulicy!',
            'Wszystkie segmenty ulicy wewnątrz rzeki. Nie mogę kontynuować.',
        ...
        case 'pt-br': // 2015-04-05: Portuguese - By esmota
          langText = new Array(
            '',
            'Largura (metros)',
            'Criar uma nova rua, selecione e clique neste botão.',
            'Rua para Rio',
            'Comprimento ilimitado (instável)',
            'Nenhuma nova rua, sem salvar, selecionada!',
            'Todos os segmentos de rua estão dentro de um rio. Nada a fazer.',
        ...
        default: // 2014-06-05: English
          langText = new Array(
            '',
            'Width (in meters)',
            'Create a new street, select and click this button.',
            'River',
            'Unlimited size (unsafe)',
            'No unsaved and selected new street found!',
            'All street segments inside river. Cannot continue.',
  ...
  function WMEKSRegisterKeyboardShortcut(a, b, c, d, e, f, g) {
    ...
    } else alert('The function ' + e + ' has not been declared');
  }
  ...
  function WMEKSSaveKeyboardShortcuts(a) {
    ...
    for (var c in W.accelerators.Actions) {
      ...
      if (W.accelerators.Actions[c].group == a) {
        W.accelerators.Actions[c].shortcut
          ? (W.accelerators.Actions[c].shortcut.altKey === !0 && (d += 'A'),
            W.accelerators.Actions[c].shortcut.shiftKey === !0 && (d += 'S'),
            W.accelerators.Actions[c].shortcut.ctrlKey === !0 && (d += 'C'),
            '' !== d && (d += '+'),
            W.accelerators.Actions[c].shortcut.keyCode && (d += W.accelerators.Actions[c].shortcut.keyCode))
          : (d = '-1');
        var e = {};
        (e[d] = W.accelerators.Actions[c].id), (b[b.length] = e);
      }
    }
